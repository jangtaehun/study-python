...
def select_by_name(**kwargs):
    name_result= {}
    if kwargs['name'] in data_dict:
      # for name, price in data_dict.items():
      #   if kwargs['name'] == name:
        name_result = {'name': kwargs['name'], 'price': data_dict[keyword]}
        # name_result = {'name': name, 'price': price}
    else:
...
def add(**kwargs):
    ...
    # if kwargs.get('new_name') in data_dict:
    if kwargs.get('new_name') not in data_dict:
        data_dict[kwargs.get('new_name')] = int(kwargs.get('new_price'))
    else:
...
def show():
    ...
    if len(data_dict) == 0:
        ...
        for name, price in data_dict.items():
            show_result.append({'name': name, 'price': price})
    return show_result
...
while True:
    ...
    if choice == 1:
        ...
        add(new_name=new_name, new_price=new_price)
    ...
    # 검색
        ...
        if choice == 1:
            ...
            result1 = select_by_name(name=keyword)
            if len(result1) != 0:
                print(f"{result1.get('name')}, {result1.get('price')}")
                continue
            else:
